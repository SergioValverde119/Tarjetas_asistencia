            ...
            <div class="p-6 bg-gray-50 min-h-full">
                ...
                <div class="mb-6 relative max-w-md">
                    ...
                    <input 
                        ...
                        placeholder="Buscar por Nombre, RFC o No. Empleado..."
                    >
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Usuario (RFC)</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Vinculación BioTime</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="user in users.data" :key="user.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span v-if="user.biotime_id" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-3">
                                            <Link :href="edit(user.id).url" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-2 rounded-lg transition-colors">
                                                <Edit class="w-4 h-4" />
                                            </Link>
                                            <button 
                                                @click="deleteUser(user)" 
                                                class="text-red-600 hover:text-red-900 bg-red-50 p-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                         ...
                         <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            ...
                            <div class="flex gap-1">
                                <Link v-for="(link, k) in users.links" :key="k" :href="link.url || '#'" v-html="link.label" 
                                    ...
                                    :class="link.active ? 'bg-blue-50 border-blue-500 text-blue-600' : 'bg-white text-gray-500'" />
                            </div>
                         </div>
                    </div>
                </div>
